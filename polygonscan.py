...
class polygonscan:
    ...
    async def csv_insert(column):
        if not exists("polygonscan.csv"):
            with open('polygonscan.csv','a+') as fd:
    ...
    async def scrap(page):
        try:
            ...
            print("Polygonscan:", columns)
            await polygonscan.csv_insert(columns) 
            polygonscan.get_db_mongo(columns)
        except:
            pass
